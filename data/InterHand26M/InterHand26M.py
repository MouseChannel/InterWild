...
class InterHand26M(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if mano_param['right'] is not None:
            ...
            rmano_joint_img, rmano_joint_cam, rmano_mesh_cam, rmano_pose, rmano_shape = get_mano_data(mano_param['right'], data['cam_param'], do_flip, img_shape)
        ...
        if mano_param['left'] is not None:
            ...
            lmano_joint_img, lmano_joint_cam, lmano_mesh_cam, lmano_pose, lmano_shape = get_mano_data(mano_param['left'], data['cam_param'], do_flip, img_shape)
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            if annot['hand_type'] == 'interacting':
                if annot['mano_param']['right'] is not None and annot['mano_param']['left'] is not None:
                    vertex_mask = np.arange(mano.vertex_num,2*mano.vertex_num)
